...
//--------------------------------------------------------------------------------------
static HRESULT CreateTextureFromDDS( _In_ ID3D12GraphicsCommandList * pCmdList,
                                     ...
                                     _In_ D3D12_CPU_DESCRIPTOR_HANDLE textureView )
{
...

    {
		...
		pCmdList->GetDevice( __uuidof(*pDevice), reinterpret_cast<void**>(&pDevice) );

        if ( SUCCEEDED(hr) )
        {
            hr = CreateD3DResources( pDevice, resDim, twidth, theight, tdepth, mipCount - skipMip, arraySize,
                                     format, forceSRGB,
		...
		if (SUCCEEDED(hr))
		{
			...
			//InitializeTexture(*texture, subresourceCount, initData.get());
		}
	...
	DDS_ALPHA_MODE* alphaMode )
{
    ...
    HRESULT hr = CreateTextureFromDDS( pCmdList,
                                       header, ddsData + offset, ddsDataSize - offset,
	...
	DDS_ALPHA_MODE* alphaMode )
{
    ...
    hr = CreateTextureFromDDS( pCmdList,
...
//void InitializeTexture( ID3D12Device * pDevice, ID3D12GraphicsCommandList * pCmdList, ID3D12Resource * Dest, UINT NumSubresources, D3D12_SUBRESOURCE_DATA SubData[] )
//{
//	ID3D12Resource* UploadBuffer;
//
//	UINT64 uploadBufferSize = GetRequiredIntermediateSize(Dest, 0, NumSubresources);
//
...
//	UpdateSubresources(InitContext.m_CommandList, Dest, UploadBuffer, 0, 0, NumSubresources, SubData);
